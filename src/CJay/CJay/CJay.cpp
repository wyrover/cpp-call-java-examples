...
inline std::string getParmPath()
{
	...
	std::string parmPath = std::string("-Djava.class.path=.;");
    return parmPath;
}

const wchar_t szJvmPath[] = L"C:\\Java\\jdk\\jre\\bin\\server\\jvm.dll";

inline jint createJavaVM(JavaVMInitArgs& vm_args, const std::wstring jvm_dll_filename)
{
    typedef jint(WINAPI * PFunCreateJavaVM)(JavaVM **, void **, void *);
    //HINSTANCE hInstance = ::LoadLibrary(szJvmPath);
	HINSTANCE hInstance = ::LoadLibrary(jvm_dll_filename.c_str());
    ...
    return (*funCreateJavaVM)(&jvm, (void**)&env, &vm_args);
...
void createVM(std::vector<std::string>& vmOption, const std::wstring jvm_dll_filename)
{
    if (env == NULL || jvm == NULL) {
        ...
        // Create JavaVM
		jint status = createJavaVM(vm_args, jvm_dll_filename);
...
void createVM(const std::wstring jvm_dll_filename)
{
    if (env == NULL || jvm == NULL) {
        JavaVMInitArgs vm_args;
...
void CJ::assignMethodLinkageCollection()
{
    ...
    for (auto& kv : this->methodReflect) {
        ...
        if (rv == "[") {
            rv = descriptor[pos + 2];    // array case
            isArray = true;
        }
        ...
        if (isArray) {
            ...
            switch (*rv.c_str()) {
            case 'Z' :
                signature = new Signature<jbooleanArray>(name, descriptor, isStatic, RV::Z);
                break;

            case 'B' :
                signature = new Signature<jbyteArray>(name, descriptor, isStatic, RV::B);
                break;

            case 'C' :
                signature = new Signature<jcharArray>(name, descriptor, isStatic, RV::C);
                break;

            case 'S' :
                signature = new Signature<jshortArray>(name, descriptor, isStatic, RV::S);
                break;

            case 'I' :
                signature = new Signature<jintArray>(name, descriptor, isStatic, RV::I);
                break;

            case 'J' :
                signature = new Signature<jlongArray>(name, descriptor, isStatic, RV::J);
                break;

            case 'F' :
                signature = new Signature<jfloatArray>(name, descriptor, isStatic, RV::F);
                break;

            case 'D' :
                signature = new Signature<jdoubleArray>(name, descriptor, isStatic, RV::D);
                break;

            case 'L' :
                signature = new Signature<jobjectArray>(name, descriptor, isStatic, RV::L);
                break;
            ...
            switch (*rv.c_str()) {
            case 'Z' :
                signature = new Signature<jboolean>(name, descriptor, isStatic, RV::Z);
                break;

            case 'B' :
                signature = new Signature<jbyte>(name, descriptor, isStatic, RV::B);
                break;

            case 'C' :
                signature = new Signature<jchar>(name, descriptor, isStatic, RV::C);
                break;

            case 'S' :
                signature = new Signature<jshort>(name, descriptor, isStatic, RV::S);
                break;

            case 'I' :
                signature = new Signature<jint>(name, descriptor, isStatic, RV::I);
                break;

            case 'J' :
                signature = new Signature<jlong>(name, descriptor, isStatic, RV::J);
                break;

            case 'F' :
                signature = new Signature<jfloat>(name, descriptor, isStatic, RV::F);
                break;

            case 'D' :
                signature = new Signature<jdouble>(name, descriptor, isStatic, RV::D);
                break;

            case 'L' :
                signature = new Signature<jobject>(name, descriptor, isStatic, RV::L);
                break;

            case 'V' :
                signature = new Signature<void>(name, descriptor, isStatic, RV::VV);
                break;
 ...
 **/
Converter::Converter(): ConverterBase()
{
    this->init();
}
Converter::~Converter() { }
...
template <> std::vector<jobject> Converter::c_cast_array(jobjectArray x)
{
    ...
    for (jsize i = 0; i < size; i++) {
        cVec.push_back(env->GetObjectArrayElement(x, i));
    }
